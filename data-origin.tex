...
we consider the following items as having the  most impact (for reuse, citation):
...
publisher         & Data centre that produced the VOTable \textit{(recommended metadata)}& publisher\\ \hline
...
retrieved \textit{(recommended metadata)}& \\ \hline
request           & Full request URL including a query string (**) \textit{(recommended metadata)}& \\ \hline
...
% end
request\_date     & Query execution date \textit{(recommended metadata)}&\\ \hline
contact           & Email or URL to contact publisher & \\ \hline
ivoid\_service    & IVOID of the service through which the data was retrieved \textit{(recommended metadata)} &  \\ \hline
...
ivoid              & IVOID of underlying data collection  \textit{(recommended metadata)} &  \\ \hline
citation           & Dataset identifier that can be used for citation (e.g. dataset DOI) \textit{(recommended metadata)}  & identifier\\ \hline
...
resource\_version  & Dataset version  \textit{(recommended metadata)} & \\ \hline
%rename 23-nov-2023 rights             & (*) Licence URI & R & rights\\ \hline
rights\_uri        & Licence URI (*) \textit{(recommended metadata)} & rights\\ \hline
...
creation of the resource; generally, the author(s)
                   \textit{(recommended metadata)} & creator\\ \hline
...
publication\_date  & Date of first publication in the data centre  (DALI timestamp) (***) \textit{(recommended metadata)}&  \\ \hline
last\_update\_date & Last data centre update (DALI timestamp) (****)  \textit{(recommended metadata)} & date\\ \hline
